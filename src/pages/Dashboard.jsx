...
function Breakdown({ title, values, to, severity = false }) { return <Section title={title} to={to}><div className="space-y-2">{Object.entries(values).sort((a,b) => b[1]-a[1]).slice(0,8).map(([label,value]) => <div key={label} className="flex justify-between text-sm"><span className="flex items-center gap-2">{severity && <span className={`w-2 h-2 rounded-full ${(SEVERITY_CONFIG[label] || "bg-slate-100").split(" ")[0]}`} />}{label}</span><strong>{value}</strong></div>)}{!Object.keys(values).length && <div className="text-sm text-slate-400">No data.</div>}</div></Section>; }
function Progress({ label, value, total, suffix = "", badge = null }) { const width = total ? Math.min(100, Math.round(Number(value) / total * 100)) : 0; return <div><div className="flex justify-between text-xs mb-1"><span>{badge || label}</span><span>{value}{suffix}</span></div><div className="h-2 bg-slate-100 rounded"><div className="h-full bg-slate-700 rounded" style={{ width: `${width}%` }} /></div></div>; }
...
function daysUntil(date) { return (new Date(`${date}T23:59:59`).getTime() - Date.now()) / 86400000; }
